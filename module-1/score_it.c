...
if(dataset==NULL)
{
	printf("Low memory or unable to load results from %s",results_file_name);
	return;
}
...
A=mlfw_mat_double_create_column_vec(dataset, 1); // 2nd column
if(A==NULL)
{
	printf("Low memory1\n");
...
P=mlfw_mat_double_create_column_vec(dataset, 2); // 3rd column
if(P==NULL)
{
	printf("Low memory2\n");
	mlfw_mat_double_destroy(dataset);
...
if(argc!=2)
{
printf("[ Usage: score_it.out resuts_file_name ]\n");
return 0;
}
